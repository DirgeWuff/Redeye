...
void renderMap(
    ...
    const Color color)
{
    assert(cam != nullptr && "cam == nullptr. Ln 15, TilemapRenderer.cpp");

    for (const std::vector<tson::Layer>& layers = map.renderDataPtr->tsonMapPtr->getLayers(); auto& layer : layers) {
        ...
        switch (layer.getType()) {
            ...
            case tson::LayerType::ImageLayer: {
                ...
                if (!map.renderDataPtr->textures.contains(imagePath)) {
                    logErr("Unable to load texture: " + layer.getImage() + ". Ln 65, TilemapRenderer.cpp.");
                    return;
                }
            ...
            default: {
                logErr("Map contains unsupported layer type. Ln 84, TilemapRenderer.cpp");
                break;
            }
        }
    }
